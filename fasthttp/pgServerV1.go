...
//
// V8:
// use hash function for command and path router
//
...
func main() {
    ...
    numarg := len(os.Args)
    flags:=[]string{"port", "index", "js", "dbg", "test", "preload"}

    useStr := " /port=portstr [/index=idxfil] [/js=startjsfile] [/preload] [/test] [/dbg]"
    ...
    jsFilnam := "azulstart.js"
    jsval, ok := flagMap["js"]
    if ok {
        if jsval.(string) == "none" {log.Fatalf("error: no js file name provided!\n")}
        jsFilnam = jsval.(string) + ".js"
    }
	...
	// substitute js start file
	idxbyt, err = insScript(idxbyt, n, jsFilnam)
	if err != nil {log.Fatalf("error -- substituting start to script file: %v\n",err)} 

	fmt.Printf("dbg -- len[%d]\n%s\n", len(idxbyt), string(idxbyt))

	res, err := parseScript(idxbyt)
	if err != nil {log.Fatalf("error -- parsing idx file: %v\n",err)} 

	han.index = idxbyt
	han.idxLen = len(idxbyt)
...
func (han Handler)jsHandler(ctx *fasthttp.RequestCtx) {
	...
	if !bytes.Equal(han.p.Ext, []byte("js"))  {
        fmt.Printf("error jsHandler -- invalid req: %s\n", ctx.Path)
...
func (han Handler)sqlHandler(ctx *fasthttp.RequestCtx) {

	if han.dbg {log.Printf("dbg -- sql method: %q index %s %s %s\n", ctx.Method(), han.p.Fold, han.p.Fnam, han.p.Ext)}


	if bytes.Equal(ctx.Method(), []byte("POST")) {
		log.Printf("info -- sqlHandler -- method post!\n")
		res := ctx.PostBody()
		log.Printf("info sqlHandler -- >: %s\n",string(res))
		return
	}
	if bytes.Equal(ctx.Method(), []byte("GET")) {
		log.Printf("info sqlHandler -- method get!\n")
		fmt.Fprintf(ctx, "Welcome to PG Server!\n")
        ctx.SetContentType("text/plain; charset=utf-8")
		return
	}

	log.Printf("error sqlHandler -- invalid method: %s", ctx.Method())
	ctx.SetStatusCode(404)
    ctx.SetContentType("text/plain; charset=utf-8")
    fmt.Fprintf(ctx, "sql -- invalid method: %s\n",ctx.Method)
	return
...
func insScript(idxbyt []byte, n int, jsFilnam string) (ridxbyt []byte, err error){

//fmt.Printf("n: %d, len isxbyt: %d\n", n, len(idxbyt))
	idxSt :=0
	idxEnd := 0
	found:= false
	for k:=0; k< 2; k++ {
		idx := bytes.Index(idxbyt[idxSt:n],[]byte("src='"))
		if idx == -1 {
			ridxbyt = idxbyt
			return ridxbyt, fmt.Errorf("cannot find script file")
		}
//fmt.Printf("found idx %d\n", idx)
		idxSt = idxSt + idx + 5

		idxL := bytes.Index(idxbyt[idxSt:n],[]byte("'>"))
		if idxL == -1 {
			ridxbyt = idxbyt
			return ridxbyt, fmt.Errorf("cannot find >")
		}
//fmt.Printf("found idxend of file: %d\n", idxL)
		idxEnd = idxSt +idxL
//fmt.Printf("--%d: %d:%d ", k, idxSt, idxEnd)
//fmt.Printf("%s\n",idxbyt[idxSt:idxEnd])
		if string(idxbyt[idxSt:idxEnd]) == "azulstart" {
			found = true
			break
		}
	...
	if !found {return idxbyt,nil}

	// save the tail
	tmp := make([]byte, 1024)
	for i:=idxEnd;i<n;i++ { 
		tmp[i-(idxEnd)] = idxbyt[i]
	}
	tmplen :=n-(idxEnd)
	fmt.Printf("tmp: %s\n", string(tmp[:tmplen]))
...
//	fmt.Printf("***begin***>\n%s\n", idxbyt[:idxSt])
	// add filnam
	filstr :="/js/"+jsFilnam
	idxbyt = append(idxbyt[:idxSt],[]byte(filstr) ...)
	l := idxSt + len(filstr)
	idxbyt = append(idxbyt[:l],tmp[:tmplen] ...) 
