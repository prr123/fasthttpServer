...
func (han Handler)Upgrader(ctx *fasthttp.RequestCtx)(err error) {
	...
	// tests
	if !bytes.Equal(upgVal,[]byte("Upgrade")) {return fmt.Errorf("Upgrade Header Value not Upgrade")}
	...
	if len(wsKeyVal) != 13 {return fmt.Errorf("Sec-WebSocket-Key Header Value length is not 13!")}
